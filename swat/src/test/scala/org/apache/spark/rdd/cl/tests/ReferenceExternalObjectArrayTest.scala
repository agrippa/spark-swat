...
object ReferenceExternalObjectArrayTest extends CodeGenTest[Float, Float] {
  def getExpectedKernel() : String = {
    ...
    "   return((((in + org_apache_spark_rdd_cl_tests_Point__x( &(this->arr$1[0]))) + org_apache_spark_rdd_cl_tests_Point__y( &(this->arr$1[1]))) + org_apache_spark_rdd_cl_tests_Point__z( &(this->arr$1[2]))));\n" +
    "}\n" +
    ...
    "__kernel void run(\n" +
    "      __global float* in0, \n" +
    "      __global float* out, __global org_apache_spark_rdd_cl_tests_Point *arr$1, int N) {\n" +
