...
object ImagenetRepartition {
    def main(args : Array[String]) {
        if (args.length != 3) {
            println("usage: ImagenetRepartition input-dir output-dir npartitions")
            return;
        }
        ...
        val output : RDD[Tuple2[Int, DenseVector]] = input.repartition(npartitions)
